...
export default function PreviousResultsPage({ theme }) {
  ...
  const doughnutData = useMemo(() => ({
    ...
    datasets: [{
      ...
      borderColor: isDark ? '#0B0E17' : '#ffffff', borderWidth: 3, hoverOffset: 10,
    }],
  }), [data, isDark]);
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-3 md:px-6 py-4">
        ...
        <div className="glass-card rounded-2xl p-5 mb-8">
          <div className="flex items-center gap-4">
            <button onClick={goPrev} disabled={yearIdx >= ELECTION_YEARS.length - 1} className={cn("p-2 rounded-xl transition-all", yearIdx >= ELECTION_YEARS.length - 1 ? "text-gray-700 cursor-not-allowed" : "hover:bg-black/10 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400")}><ChevronLeft className="w-5 h-5" /></button>
              ...
              {ELECTION_YEARS.map(y => (
                <button key={y} onClick={() => setSelectedYear(y)}
                  className={cn("px-5 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap border",
                    selectedYear === y ? "bg-amber-500/20 border-amber-500/30 text-amber-400 shadow-lg shadow-amber-500/10" : "bg-black/[0.01] dark:bg-white/[0.02] border-black/5 dark:border-white/5 text-gray-500 dark:text-gray-400 hover:bg-black/5 dark:hover:bg-white/5"
                  )}>
                  <Calendar className="w-3.5 h-3.5 inline mr-1.5" />{y}
                </button>
              ))}
            </div>
            <button onClick={goNext} disabled={yearIdx <= 0} className={cn("p-2 rounded-xl transition-all", yearIdx <= 0 ? "text-gray-700 cursor-not-allowed" : "hover:bg-black/10 dark:hover:bg-white/10 text-gray-500 dark:text-gray-400")}><ChevronRight className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 mb-8 relative overflow-hidden" style={{ borderLeft: `4px solid ${PARTY_HISTORY_COLORS[winner?.party] || '#6b7280'}` }}>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex gap-6">
              <div className="text-center"><div className="text-2xl font-black text-[var(--text-primary)]">{winner?.seats}</div><div className="text-[10px] text-gray-500 uppercase">Seats Won</div></div>
              <div className="text-center"><div className="text-2xl font-black text-[var(--text-primary)]">{winner?.voteShare}%</div><div className="text-[10px] text-gray-500 uppercase">Vote Share</div></div>
              <div className="text-center"><div className="text-2xl font-black text-[var(--text-primary)]">{data.turnout}</div><div className="text-[10px] text-gray-500 uppercase">Turnout</div></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="h-[300px]"><Bar data={barData} options={{ responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false }, tooltip: chartTooltip }, scales: { x: { grid: { color: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.06)' }, ticks: { color: isDark ? '#9ca3af' : '#4b5563' } }, y: { grid: { display: false }, ticks: { color: isDark ? '#9ca3af' : '#4b5563', font: { size: 12, weight: 'bold' } } } } }} /></div>
          </div>
          {/* Vote Share Doughnut */}
          <div className="glass-card rounded-2xl p-6">
              ...
              <div className="flex-1 space-y-2 w-full">
                {data.parties.map(p => (
                  <div key={p.party} className="flex items-center gap-2 text-sm">
                    ...
                    <div className="flex-1 bg-gray-200 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${p.voteShare * 2}%`, backgroundColor: PARTY_HISTORY_COLORS[p.party] || '#6b7280' }} /></div>
                ...
                {data.parties.map(p => {
                  ...
                  return (
                    <tr key={p.party} className="table-row-transition">
                      ...
                      <td className="p-4 text-center"><div className="flex items-center justify-center gap-2"><div className="w-16 bg-gray-200 dark:bg-gray-800 rounded-full h-1.5 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${p.voteShare * 2}%`, backgroundColor: color }} /></div><span className="text-xs font-mono text-gray-400">{p.voteShare}%</span></div></td>
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {data.keyConstituencies.map((c, i) => (
                <div key={i} className="flex items-center gap-3 bg-black/[0.01] dark:bg-white/[0.02] p-4 rounded-xl border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{c.winner} · <span style={{ color: PARTY_HISTORY_COLORS[c.party] }}>{c.party}</span></div>
                  </div>
                  <div className="text-right shrink-0"><div className="text-green-600 dark:text-green-400 font-mono font-bold text-sm">+{c.margin.toLocaleString()}</div><div className="text-[9px] text-gray-500 uppercase">margin</div></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {data.highlights.map((h, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-black/[0.01] dark:bg-white/[0.02] rounded-xl border border-black/5 dark:border-white/5">
        ...
        <div className="glass-card rounded-2xl p-6 mb-8">
          ...
          <div className="h-[320px]"><Line data={trendLine} options={{ responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { labels: { color: isDark ? '#9ca3af' : '#4b5563', font: { size: 11 }, usePointStyle: true } }, tooltip: chartTooltip }, scales: scaleOpts }} /></div>
        </div>
              ...
              return (
                <button key={y} onClick={() => setSelectedYear(y)}
                  className={cn("text-left p-5 rounded-xl border transition-all hover:scale-[1.02] cursor-pointer",
                    selectedYear === y ? "bg-black/10 dark:bg-white/10 border-black/20 dark:border-white/20 shadow-lg" : "bg-black/[0.01] dark:bg-white/[0.02] border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5"
                  ...
                  <div className="text-lg font-black text-[var(--text-primary)]">{w?.seats} <span className="text-xs text-gray-500 font-normal">seats</span></div>
        ...
        <footer className="text-center py-6 border-t border-black/5 dark:border-white/5"><p className="text-[11px] text-gray-500">Historical Data · Tamil Nadu / Madras State Assembly Elections · 1952–2021</p></footer>
      </div>
    </div>
  );
}
