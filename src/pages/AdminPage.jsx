...
export default function AdminPage({ liveSeats, setLiveSeats, voteShares, setVoteShares, constituencies, setConstituencies, simulationOn, setSimulationOn }) {
  ...
  // Login screen
  if (!authed) return (
    <div className="min-h-screen bg-[#0B0E17] text-gray-200 flex items-center justify-center">
      ...
      <div className="glass-card rounded-3xl p-10 w-full max-w-md relative z-10 text-center">
        ...
        <form onSubmit={handleLogin}>
          <input type="password" value={pin} onChange={e => { setPin(e.target.value); setPinError(false); }} placeholder="Enter PIN"
            className={cn("w-full bg-gray-800/60 border text-center text-2xl tracking-[0.5em] rounded-xl px-6 py-4 outline-none focus:ring-2 ring-red-500 mb-4 transition-all", pinError ? "border-red-500 shake" : "border-white/10")} autoFocus />
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setSimulationOn(!simulationOn)}
              className={cn("flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all border", simulationOn ? "bg-green-500/10 border-green-500/20 text-green-400" : "bg-red-500/10 border-red-500/20 text-red-400")}>
              {simulationOn ? <Play className="w-3.5 h-3.5" /> : <Pause className="w-3.5 h-3.5" />}
              Simulation {simulationOn ? 'ON' : 'OFF'}
            </button>
            <button onClick={exportData} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-gray-800 hover:bg-gray-700 border border-white/10 transition-all"><Download className="w-3.5 h-3.5" /> Export</button>
            <label className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-gray-800 hover:bg-gray-700 border border-white/10 transition-all cursor-pointer"><Upload className="w-3.5 h-3.5" /> Import<input type="file" accept=".json" onChange={importData} className="hidden" /></label>
        ...
        <div className="flex gap-2 mb-6">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={cn("flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all border", tab === t.id ? "bg-white/10 border-white/20 text-white" : "bg-white/[0.02] border-white/5 text-gray-400 hover:bg-white/5")}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
            ...
            {showForm && (
              <div className="glass-card rounded-2xl p-6 border border-purple-500/20">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Name *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500" placeholder="e.g. Chennai Central" /></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Candidate *</label><input value={form.candidate} onChange={e => setForm({ ...form, candidate: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500" placeholder="e.g. John Doe" /></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Party</label><select value={form.party} onChange={e => setForm({ ...form, party: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500">{PARTIES.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">District</label><select value={form.district} onChange={e => setForm({ ...form, district: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500">{DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Margin</label><input type="number" value={form.margin} onChange={e => setForm({ ...form, margin: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500" /></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Rounds (0-18)</label><input type="number" min="0" max="18" value={form.rounds} onChange={e => setForm({ ...form, rounds: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500" /></div>
                  <div><label className="text-xs text-gray-500 uppercase mb-1 block">Status</label><select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })} className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 ring-red-500">{STATUSES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {constituencies.map((c, i) => (
                      <tr key={i} className="table-row-transition">
                        ...
                        <td className="p-4 text-gray-300">{c.candidate}</td>
                        ...
                        <td className="p-4 text-center text-gray-400">{c.district}</td>
                        <td className="p-4 text-right font-mono text-green-400">+{Number(c.margin).toLocaleString()}</td>
                        <td className="p-4 text-center text-gray-400">{c.rounds}/18</td>
                        ...
                        <td className="p-4"><div className="flex items-center justify-center gap-2"><button onClick={() => editRow(i)} className="p-1.5 rounded-lg hover:bg-white/10 text-blue-400 transition-colors"><Edit3 className="w-3.5 h-3.5" /></button><button onClick={() => deleteRow(i)} className="p-1.5 rounded-lg hover:bg-white/10 text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 'seats' && (
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {PARTIES.map(p => (
                <div key={p} className="bg-white/[0.02] rounded-xl p-5 border border-white/5" style={{ borderLeftColor: PARTY_COLORS[p], borderLeftWidth: '3px' }}>
                  ...
                  <input type="number" min="0" max="234" value={liveSeats[p] || 0} onChange={e => setLiveSeats(prev => ({ ...prev, [p]: Number(e.target.value) }))}
                    className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-3 text-2xl font-black text-center outline-none focus:ring-2 transition-all" style={{ color: PARTY_COLORS[p], ringColor: PARTY_COLORS[p] }} />
                </div>
              ))}
            </div>
            <div className="mt-6 p-4 rounded-xl bg-black/30 border border-white/5 flex items-center justify-between">
              ...
              <span className={cn("text-2xl font-black", Object.values(liveSeats).reduce((a, b) => a + b, 0) === 234 ? "text-green-400" : "text-red-400")}>{Object.values(liveSeats).reduce((a, b) => a + b, 0)} / 234</span>
            </div>
          </div>
        )}
                ...
                <div key={p} className="bg-white/[0.02] rounded-xl p-5 border border-white/5" style={{ borderLeftColor: PARTY_COLORS[p], borderLeftWidth: '3px' }}>
                  ...
                  <div className="relative">
                    <input type="number" step="0.1" min="0" max="100" value={Number(voteShares[p] || 0).toFixed(1)} onChange={e => setVoteShares(prev => ({ ...prev, [p]: Number(e.target.value) }))}
                      className="w-full bg-gray-800/60 border border-white/10 rounded-xl px-4 py-3 text-2xl font-black text-center outline-none focus:ring-2 transition-all pr-10" style={{ color: PARTY_COLORS[p] }} />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 font-bold">%</span>
                  </div>
                  <div className="w-full bg-gray-800 rounded-full h-1.5 mt-3 overflow-hidden"><div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.min(100, (voteShares[p] || 0) * 2)}%`, backgroundColor: PARTY_COLORS[p] }} /></div>
                </div>
              ))}
            </div>
            <div className="mt-6 p-4 rounded-xl bg-black/30 border border-white/5 flex items-center justify-between">
              <span className="text-sm text-gray-400">Total Vote Share</span>
              <span className={cn("text-2xl font-black", Math.abs(Object.values(voteShares).reduce((a, b) => a + Number(b), 0) - 100) < 1 ? "text-green-400" : "text-yellow-400")}>{Object.values(voteShares).reduce((a, b) => a + Number(b), 0).toFixed(1)}%</span>
            </div>
          </div>
        )}

        <footer className="text-center py-6 mt-8 border-t border-white/5"><p className="text-[11px] text-gray-500">Admin Panel · Election Dashboard TN 2026</p></footer>
      </div>
    </div>
  );
}
