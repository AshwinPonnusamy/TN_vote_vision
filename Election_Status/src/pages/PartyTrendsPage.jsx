...
const chartTooltip = {
  backgroundColor: 'rgba(11,14,23,0.9)', borderColor: 'rgba(255,255,255,0.1)',
  borderWidth: 1, padding: 12, cornerRadius: 12,
};

export default function PartyTrendsPage({ liveSeats, voteShares }) {
  ...
  const doughnut = {
    labels: PARTIES,
    datasets: [{ data: PARTIES.map(p => liveSeats[p] || 0), backgroundColor: PARTIES.map(p => PARTY_COLORS[p]), borderColor: '#0B0E17', borderWidth: 3, hoverOffset: 12 }],
  };
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-3 md:px-6 py-4">
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="flex-1 space-y-3 w-full">
                {PARTIES.map(p => { const s = liveSeats[p] || 0, pct = total > 0 ? ((s / total) * 100).toFixed(1) : 0; return (
                  <div key={p} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, backgroundColor: PARTY_COLORS[p] }} /></div>
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="h-[320px]"><Line data={line} options={{ responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { labels: { color: '#9ca3af', font: { size: 11 }, usePointStyle: true } }, tooltip: chartTooltip }, scales: scaleOpts }} /></div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="h-[320px]"><Radar data={radar} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#9ca3af', font: { size: 11 }, usePointStyle: true } } }, scales: { r: { grid: { color: 'rgba(255,255,255,0.06)' }, angleLines: { color: 'rgba(255,255,255,0.06)' }, pointLabels: { color: '#9ca3af', font: { size: 10 } }, ticks: { display: false } } } }} /></div>
          </div>
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="h-[320px]"><Bar data={histBar} options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#9ca3af', font: { size: 10 }, usePointStyle: true }, position: 'top' }, tooltip: chartTooltip }, scales: scaleOpts }} /></div>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 mb-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {PARTIES.map(p => { const s = liveSeats[p] || 0, vs = (voteShares[p] || 0).toFixed(1); return (
              <div key={p} className="bg-white/[0.02] rounded-xl p-5 border border-white/5 hover:border-white/10 transition-all" style={{ borderLeftColor: PARTY_COLORS[p], borderLeftWidth: '3px' }}>
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-black/20 rounded-lg p-2"><div className="text-green-400 font-bold text-sm">{Math.floor(s * 0.6)}</div><div className="text-[9px] text-gray-500 uppercase">Won</div></div>
                  <div className="bg-black/20 rounded-lg p-2"><div className="text-blue-400 font-bold text-sm">{Math.floor(s * 0.4)}</div><div className="text-[9px] text-gray-500 uppercase">Leading</div></div>
                  <div className="bg-black/20 rounded-lg p-2"><div className="text-gray-300 font-bold text-sm">{vs}%</div><div className="text-[9px] text-gray-500 uppercase">Vote %</div></div>
                </div>
                <div className="w-full bg-gray-800 rounded-full h-1.5 mt-4 overflow-hidden"><div className="h-full rounded-full transition-all duration-700" style={{ width: `${(s / 234) * 100}%`, backgroundColor: PARTY_COLORS[p] }} /></div>
              </div>
            ); })}
          </div>
        </div>

        <footer className="text-center py-6 border-t border-white/5"><p className="text-[11px] text-gray-500">Live counting trends · Auto-refreshes every 3s · TN Assembly 2026</p></footer>
      </div>
    </div>
  );
}
